for k = 1 : 3
    ...
    for l = 1:10
    %% 初始化任务
    choose_case = k; % 选择参数组数（1，2或者3）
    use = 2; % 任务类型(物料加工所需工序数目，1或者2)
    global mode;
    ...
    % CNC(:, 2) = [1, 1, 1, 1 ,2, 2, 2, 2];
    % CNC(:, 2) = [1, 2, 1, 2 ,1, 2, 2, 2];
    % CNC(:, 2) = [1, 2, 1, 2 ,1, 2, 1, 1];
    % CNC(:, 2) = [1, 2, 1, 2 ,2, 2, 1, 2];
    % CNC(:, 2) = [1, 2, 1, 2 ,1, 1, 1, 2];
    % CNC(:, 2) = [1, 1, 1, 2 ,2, 2, 2, 2];
     CNC(:, 2) = [1, 1, 1, 1 ,1, 2, 2, 2];
    time = 0; % 时间初始化
    ...
    while time <= 28800    % 8小时等于28800分钟
        ...
        if use == 1
            [target, cost,road] = Caseone(RGV,CNC,Case(:, :,choose_case));
        else
            [target, cost, road] = Casetwo(RGV,RGV_num,CNC,Case(:,:,choose_case));
        end
        if time + cost > 28800
            break;
        end 
